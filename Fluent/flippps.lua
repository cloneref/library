...
function Library:SafeCallback(Function, ...)
	...
	if not Success then
		Event = tostring(Event or "Unknown error")
	...
	return function(Config)
		...
		function Window:Minimize()
			...
			if not Mobile then Library:Notify({
				Title = "Interface",
				Content = "Press " .. tostring(Key or "Unknown") .. " to toggle the interface.",
				Duration = 6
				})
				else 
					Library:Notify({
...
local SaveManager = {} do
	...
	Vector3Position = {
		...
		Save = function(idx, object)
			...
			if object.Value and typeof(object.Value) == "Vector3" then
		...
		Load = function(idx, data)
			...
			if SaveManager.Options[idx] and data.x and data.y and data.z then


				local vec = Vector3.new(data.x, data.y, data.z)


				SaveManager.Options[idx]:SetValue(vec)
	...
	function SaveManager:LoadAutoloadConfig()
		...
		if isfile(tostring(self.Folder or "FluentSettings") .. "/autoload.txt") then


			local name = readfile(tostring(self.Folder or "FluentSettings") .. "/autoload.txt")
				...
				if not success then
					...
					return self.Library:Notify({
						...
						SubContent = "Failed to load autoload config: " .. tostring(err or "Unknown error"),
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddButton({
			...
			Callback = function()
				...
				local name = SaveManager.Options.SaveManager_ConfigName.Value or ""
			name = tostring(name)
				...
				if not success then
					...
					return self.Library:Notify({
						...
						SubContent = "Failed to save config: " .. tostring(err or "Unknown error"),
		...
		section:AddButton({Title = "Load config", Callback = function()
			...
			if not success then
				...
				return self.Library:Notify({
					...
					SubContent = "Failed to load config: " .. tostring(err or "Unknown error"),
		...
		section:AddButton({Title = "Save config", Callback = function()
			...
			if not success then
				...
				return self.Library:Notify({
					...
					SubContent = "Failed to overwrite config: " .. tostring(err or "Unknown error"),
		...
		writefile(tostring(self.Folder or "FluentSettings") .. "/autoload.txt", name)
...
local InterfaceManager = {} do
	...
	function InterfaceManager:SaveSettings()
		...
		writefile(tostring(self.Folder or "FluentSettings") .. "/options.json", httpService:JSONEncode(InterfaceManager.Settings))
	...
	function InterfaceManager:LoadSettings()
		...
		local path = tostring(self.Folder or "FluentSettings") .. "/options.json"
